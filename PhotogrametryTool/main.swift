...
struct PhotogrametryTool: ParsableCommand {
    ...
    func run() {
        ...
        if (inputFolder.hasSuffix(".usdz")) {
            convertToObj(inputFile: URL(fileURLWithPath: inputFolder))
        } else {
            ...
            actor Bar {
                var progressBar = ProgressBar(count: 100, configuration: [ProgressPercent(), ProgressBarLine(barLength: 30)])
                    ...
                    for try await output in session.outputs {
                        switch output {
                            ...
                            case .requestComplete(let request, let result):
                                PhotogrametryTool.handleRequestComplete(request: request, result: result)
                            case .requestProgress(let request, let fractionComplete):
                                await bar.setValue(Int(fractionComplete * 100))
                                PhotogrametryTool.handleRequestProgress(request: request,
    ...
    private func makeRequestFromArguments() -> PhotogrammetrySession.Request {
        let outputUrl = URL(fileURLWithPath: outputFilename+".usdz")
    ...
    private func convertToObj(inputFile: URL) {
        let modelAsset = MDLAsset(url: inputFile)
        modelAsset.loadTextures()
        do {
            try modelAsset.export(to:URL(fileURLWithPath: outputFilename+".obj"))
